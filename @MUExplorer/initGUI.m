...
if isempty(obj.Figure) || ~isvalid(obj.Figure)
    ...
    obj.MetaTextHandles.SyncStats = uicontrol(obj.MetaPanel, ...
        'Style', 'text', ...
        'String', 'Synchronization: N/A', ...
        'Units', 'normalized', ...
        'Position', [0.01 0.20 0.98 0.15], ...
        'FontSize', 9, ...
        'HorizontalAlignment', 'left');


    obj.MetaWidgets.ShiftButton = uicontrol(obj.MetaPanel, ...
        'Style', 'pushbutton', ...
        'String', 'Shift Spikes...', ...
        'Units', 'normalized', ...
        'Position', [0.01 0.01 0.98 0.08], ...
        'FontSize', 10, ...
        'Callback', @(src, evt) obj.shiftSpikesDialog());
    ...
    if ~isempty(obj.PathTrace) || ~isvalid(obj.PathTrace)
        yy = obj.PathTrace * obj.Spacing * 0.25 * nCh + obj.Spacing*0.1*nCh;
        obj.PathTraceHandle = plot(obj.MainAxes, obj.Time(1:numel(yy)), yy, 'LineStyle','-',...
            'Color', [0.65 0.65 0.65], 'LineWidth', 2, ...
            'HitTest','off');
    end
    if ~isempty(obj.RefSignal) || ~isvalid(obj.RefSignal)
        yy = obj.RefSignal * 0.2 * obj.Spacing * nCh + 0.75*obj.Spacing*nCh;
        obj.SyncTraceHandle = plot(obj.MainAxes, obj.Time(1:numel(yy)), yy, 'LineStyle', '-', ...
